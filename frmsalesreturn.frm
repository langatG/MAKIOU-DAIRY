...
Begin VB.Form frmsalesreturn 
   ...
   Begin VB.ComboBox cbocategory 
      ...
      EndProperty
      Height          =   315
      ItemData        =   "frmsalesreturn.frx":0000
      Left            =   8040
      List            =   "frmsalesreturn.frx":000D
      TabIndex        =   62
      Top             =   1800
      Width           =   2775
   End
   ...
   Begin VB.PictureBox Picture2 
      ...
      Height          =   360
      Left            =   4080
      Picture         =   "frmsalesreturn.frx":0037
      ScaleHeight     =   360
      ScaleWidth      =   240
      TabIndex        =   19
      Top             =   120
   ...
   Begin VB.PictureBox Picture1 
      ...
      Height          =   360
      Left            =   4080
      Picture         =   "frmsalesreturn.frx":01B9
      ScaleHeight     =   360
      ScaleWidth      =   240
      TabIndex        =   18
      Top             =   600
   ...
   Begin VB.ComboBox Cmbstation 
      ...
      EndProperty
      Height          =   315
      ItemData        =   "frmsalesreturn.frx":033B
      Left            =   8040
      List            =   "frmsalesreturn.frx":033D
      TabIndex        =   9
      Top             =   1320
      Width           =   3015
   End
   Begin VB.OptionButton Optbranch 
      Caption         =   "Return From Station"
      Height          =   375
      Left            =   6480
      TabIndex        =   8
      Top             =   1320
   ...
   Begin MSComctlLib.ListView Lvwitems 
      ...
      Appearance      =   1
      MousePointer    =   4
      MouseIcon       =   "frmsalesreturn.frx":033F
      NumItems        =   6
...
sql = sql & "," & Lvwitems.SelectedItem.SubItems(2) & "," & Remain & ",'" & User & "'," & Lvwitems.SelectedItem.SubItems(5) & ",'" & centre & "','Dispatch to station','Sales Returned'+ + ' " & txtComment & "')"
...
If Not rs2.EOF Then
DRaccno = IIf(IsNull(rs2.Fields(1)), centre, rs2.Fields(1))
Craccno = IIf(IsNull(rs2.Fields(0)), centre, rs2.Fields(0))
Else
DRaccno = centre
Craccno = centre
End If
'XXXXXXXXXXX SAVE TO GL
    sql = ""
    sql = "set dateformat dmy insert into gltransactions(transdate,amount,draccno,craccno,documentno,source,transdescript,auditid,cash,doc_posted) values('" & txtransdate & "'," & Amount & ",'" & DRaccno & "','" & Craccno & "','" & Lvwitems.SelectedItem & "','" & Lvwitems.SelectedItem.SubItems(1) & "' ,'SALES RETURN ','" & User & "',0,0)"
    oSaccoMaster.ExecuteThis (sql)
Next j
...
sql = sql & "," & Lvwitems.SelectedItem.SubItems(2) & "," & Remain & ",'" & User & "'," & Lvwitems.SelectedItem.SubItems(5) & ",'" & txttranscode & "','checkoff','Sales Returned'+ + ' " & txtComment & "')"
...
'Loop
 sql = ""
    sql = "set dateformat dmy insert into gltransactions(transdate,amount,draccno,craccno,documentno,source,transdescript,auditid,cash,doc_posted) values('" & txtransdate & "'," & Lvwitems.SelectedItem.SubItems(4) & ",'I013','A047','" & Lvwitems.SelectedItem & "','" & Lvwitems.SelectedItem.SubItems(1) & "' ,' CHECK OFF SALES RETURN','" & User & "',0,0)"
    oSaccoMaster.ExecuteThis (sql)
...
If Optcash.Value = False Then
Set cn = New ADODB.Connection
sql = "d_sp_TransDeduct '" & txttranscode & "','" & txtransdate & "','Agrovet'," & -1 * Total & ",'" & Startdate & "','" & Enddate & "','" & User & "','Receiptno " & txtrno & "','0'"
oSaccoMaster.ExecuteThis (sql)
End If
...
Set cn = New ADODB.Connection
sql = "d_sp_TransDeduct '" & txttranscode & "','" & txtransdate & "','Agrovet'," & Amount & ",'" & Startdate & "','" & Enddate & "','" & User & "','Cash','0'"
oSaccoMaster.ExecuteThis (sql)
End If
     ' REFLECT CASH PARTLY SALES HERE
sql = ""
    sql = "set dateformat dmy insert into gltransactions(transdate,amount,draccno,craccno,documentno,source,transdescript,auditid,cash,doc_posted) values('" & txtransdate & "'," & Amount & ",'I014','A046','" & Lvwitems.SelectedItem & "','" & Lvwitems.SelectedItem.SubItems(1) & "' ,'PARTLY CASH CHECK OFF SALES RETURN ','" & User & "',0,0)"
    oSaccoMaster.ExecuteThis (sql)
    ' REDUCE CREDIT SALES HERE
    sql = ""
    sql = "set dateformat dmy insert into gltransactions(transdate,amount,draccno,craccno,documentno,source,transdescript,auditid,cash,doc_posted) values('" & txtransdate & "'," & Amount & ",'A047','I013','" & Lvwitems.SelectedItem & "','" & Lvwitems.SelectedItem.SubItems(1) & "' ,'PARTLY CASH CHECK OFF SALES RETURN','" & User & "',0,0)"
    oSaccoMaster.ExecuteThis (sql)
oSaccoMaster.ExecuteThis (sql)
...
sql = sql & "SET dateformat DMY INSERT INTO ag_Receipts(R_No, P_code, T_Date, Amount, Qua, S_Bal, user_id, Cash, SNo,Remarks,Description) VALUES ("
sql = sql & txtrno & ",'" & Lvwitems.SelectedItem & "','" & txtransdate & "'," & -1 * Lvwitems.SelectedItem.SubItems(4)
sql = sql & "," & Lvwitems.SelectedItem.SubItems(2) & "," & Remain & ",'" & User & "'," & Lvwitems.SelectedItem.SubItems(5) & ",'" & C & "','cash','Sales Returned'+ + ' " & txtComment & "' )"
...
'\\ save to gl
    ...
    sql = ""
    sql = "set dateformat dmy insert into gltransactions(transdate,amount,draccno,craccno,documentno,source,transdescript,auditid,cash,doc_posted) values('" & txtransdate & "'," & Lvwitems.SelectedItem.SubItems(4) & ",'I014','A046','" & Lvwitems.SelectedItem & "','" & Lvwitems.SelectedItem.SubItems(1) & "' ,'Cash Sales Return','" & User & "',1,0)"
    oSaccoMaster.ExecuteThis (sql)
'
...
sql = sql & txtrno & ",'" & Lvwitems.SelectedItem & "','" & txtransdate & "'," & -1 * Lvwitems.SelectedItem.SubItems(4)
sql = sql & "," & Lvwitems.SelectedItem.SubItems(2) & "," & Remain & ",'" & User & "'," & Lvwitems.SelectedItem.SubItems(5) & ",'" & txtSNo & "','" & txtSNo & "','checkoff','Sales Returned'+ + ' " & txtComment & "')"
...
If Optcash.Value = False Then
Set cn = New ADODB.Connection
sql = "d_sp_SupplierDeduct '" & txtSNo & "','" & txtransdate & "','Agrovet'," & -1 * Total & ",'" & Startdate & "','" & Enddate & "'," & year(txtransdate) & ",'" & User & "','Receipt " & txtrno & "'"
oSaccoMaster.ExecuteThis (sql)
End If
...
If CDbl(txtamtreceived) > 0 Then
    ...
    sql = "d_sp_SupplierDeduct '" & txtSNo & "','" & txtransdate & "','Agrovet'," & -1 * Amount & ",'" & Startdate & "','" & Enddate & "'," & year(txtransdate) & ",'" & User & "','Cash'"
...
Private Sub PrintReceipt()
    ...
    Printer.Print Tab(0); "STOCK RETURNED FROM " & centre & ""
    Else
     Printer.Print Tab(0); "SALES RETURNED"
    End If
    If lblCheckOff = True Then
...
sql = sql & "SET dateformat DMY INSERT INTO ag_Receipts(R_No, P_code, T_Date, Amount, Qua, S_Bal, user_id, Cash, SNo,Remarks,Description) VALUES ("
sql = sql & txtrno & ",'" & Lvwitems.SelectedItem & "','" & txtransdate & "'," & -1 * Lvwitems.SelectedItem.SubItems(4)
sql = sql & "," & Lvwitems.SelectedItem.SubItems(2) & "," & Remain & ",'" & User & "'," & Lvwitems.SelectedItem.SubItems(5) & ",'" & C & "','Credit','Sales Returned'+ + ' " & txtComment & "')"

oSaccoMaster.ExecuteThis (sql)
oSaccoMaster.ExecuteThis ("Update ag_Products SET Qout =" & CCur(Remain) & " WHERE p_code= '" & Lvwitems.SelectedItem & "'")
'//XXXXXXXXXXXXXXX
    sql = ""
    sql = "set dateformat dmy insert into gltransactions(transdate,amount,draccno,craccno,documentno,source,transdescript,auditid,cash,doc_posted) values('" & txtransdate & "'," & Lvwitems.SelectedItem.SubItems(4) & ",'I013','A048','" & Lvwitems.SelectedItem & "','" & Lvwitems.SelectedItem.SubItems(1) & "' ,' STAFF SALES ','" & User & "',0,0)"
    oSaccoMaster.ExecuteThis (sql)
